...
class ColabDisplay:
    ...
    def _create_controls(self):
        ...
        # Zoom buttons
        self.btn_zoom_in = widgets.Button(description='Zoom In', style={'button_color': '#4CAF50'})
        self.btn_zoom_out = widgets.Button(description='Zoom Out', style={'button_color': '#f44336'})
        
        # Quit button
        self.btn_quit = widgets.Button(description='QUIT', style={'button_color': '#ff0000'})
        
        # Bind events
        self.btn_up.on_click(lambda b: self._set_key('up', True))
        self.btn_up.on_click(lambda b: self._release_key('up'), type='dblclick')
        
        self.btn_down.on_click(lambda b: self._set_key('down', True))
        self.btn_down.on_click(lambda b: self._release_key('down'), type='dblclick')
        
        self.btn_left.on_click(lambda b: self._set_key('left', True))
        self.btn_left.on_click(lambda b: self._release_key('left'), type='dblclick')
        
        self.btn_right.on_click(lambda b: self._set_key('right', True))
        self.btn_right.on_click(lambda b: self._release_key('right'), type='dblclick')
        ...
        controls = widgets.VBox([
            ...
            zoom_box,
            quit_box,
            widgets.HTML(value='<p><b>Instructions:</b> Click arrows to orbit, zoom buttons to move closer/farther</p>')
        ])

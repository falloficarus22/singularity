...
def rk4_step(state, E, dl):
    ...
    return state + (dl / 6.0) * (kl + 2.0*k2 + 2.0*k3 + k4)
...
# Ray Tracing
def trace_ray(pos, direction, dl = D_LAMBDA, max_steps = MAX_STEPS, escape_r = ESCAPE_R):
...
def plot_geodesics_2d(rays_data):
    ...
    # Photon Sphere
    ps = Circle((0, 0), 1.5 * r_s, fill = False, ec = '#44ddff',
        linewidth = 1, linestyle = "--", alpha = 0.4, zorder = 5)
    ax.add_patch(ps)

    # Innermost Stable Circular Orbit
    isco = Circle((0, 0), 3.0 * r_s, fill = False, ec = '#44aaff',
        linewidth = 1, linestyle = ":", alpha = 0.3, zorder = 5)
    ax.add_patch(isco)

    # Accretion Disk Edges
    for r_disk in [r_s * 2.2, r_s * 5.2]:
        disk = Circle((0, 0), r_disk, fill = False, ec = "#ff8800",
            linewidth = 0.8, linestyle = ":", alpha = 0.25, zorder = 5)
    ax.add_patch(disk)
    ...
    ax.set_title("Schwartzschild Geodesics = Light Bending Around a Black Hole",
    ...
    legend_elements = [
        Line2D([0], [0], color='#ff4444', linewidth=2, label=f'Event Horizon  r_s = {r_s:.3e} m'),
        Line2D([0], [0], color='#ffdd44', linewidth=1, linestyle='--', label='Photon Sphere  r = 1.5 r_s'),
        Line2D([0], [0], color='#44aaff', linewidth=1, linestyle=':', label='ISCO  r = 3 r_s'),
        Line2D([0], [0], color='#ff3333', linewidth=1.5, label='Captured rays'),
        Line2D([0], [0], color='#44ddff', linewidth=1.5, label='Escaped rays')
    ]
    ax.legend(handles = legend_elements, loc = "upper_left", fontsize = 9,
        facecolor = "#1a1f2e", edgecolor = "#444444", labelcolor = "white")

    plt.tight_layout()
    plt.savefig("geodesics_2d.png", dp = 200, facecolor = fig.get_facecolor(), bbox_inches = "tight")
    plt.show()
    print("Image saved")
...
def plot_geodesics_3d(rays_data):
    ...
    for i, (trail, fate) in enumerate(rays_data):
        ...
        ax.plot(trail[::step, 0], trail[::step, 1], trail[::step, 2],
    ...
    plt.tight_layout()
    plt.savefig('geodesics_3d.png', dpi=200, facecolor=fig.get_facecolor(), bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print(f"              ({cam_distacne / r_s:.1f} r_s from center)")
    print()
    ...
    print(f" Impact Parameters: {offsets[0]/r_s:+.1f} r_s -> {offsets[-1]/r:+.1f} r_s")
    print()
